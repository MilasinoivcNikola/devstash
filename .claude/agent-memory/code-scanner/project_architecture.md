...
- `mockItemTypes`, `mockCollections`, `mockItems`, `mockItemTypeCounts` exports in mock-data.ts are dead code (not imported anywhere)
...
- Dashboard layout fires 2 DB queries; dashboard page fires 5 — all parallelized via Promise.all (good pattern)
- Prisma schema: `datasource db` block has no `url` field (relies on prisma.config.ts for URL injection — Prisma 7 pattern)
- `prisma/seed.ts` hardcodes demo password "12345678" — acceptable for seed data, not a production concern
- Tailwind v4 CSS-based config is correctly implemented (no tailwind.config.ts present)
- `TopBar.tsx` is not a client component but receives an onClick prop — works because the prop is passed from the client parent `DashboardShell`
